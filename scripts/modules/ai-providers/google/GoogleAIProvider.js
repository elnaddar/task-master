...
/**
 * Google AI provider implementation using Gemini
 */
export default class GoogleAIProvider extends AIProvider {
  #client = null;
  #model = null;
  #config = null;
  ...
  async generateTasks(prd, options = {}) {
    ...
    const prompt = `You are an AI assistant helping to break down a Product Requirements Document (PRD) into a set of sequential development tasks. 
...
Respond ONLY with the JSON output, no additional text.`;
  ...
  async expandTask(task, options = {}) {
    const { numSubtasks = 3, prompt: additionalContext = '' } = options;

    const prompt = `You are an AI assistant helping to break down a development task into smaller subtasks.
...
Respond ONLY with the JSON output, no additional text.`;
  ...
  async analyzeComplexity(task) {
    const prompt = `You are an AI assistant analyzing the complexity of a development task.
  ...
  handleError(error) {
    ...
    if (error.status) {
      switch (error.status) {
        ...
        default:
          return `Google AI API error: ${error.message}`;
      }
    }
    ...
    return `Error communicating with Google AI: ${error.message}`;
  }
